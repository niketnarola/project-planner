...
<div {{ $attributes->class(['filament-page']) }}>
	<div class="space-y-6">
	    ...
	    @if ($headerWidgets = $this->getVisibleHeaderWidgets())
		    ...
		    >
		    	@forelse( $plan_categories as $k => $category )
			        @foreach ($headerWidgets as $widget)
				    	@php
				    		$widgetData['tableHeading'] = $category;
				    		$widgetData['tableHeadingId'] = $k;
				    		$widgetData['record'] = $record;
				    	@endphp

			            @if ($widget::canView())
			                @livewire(\Livewire\Livewire::getAlias($widget), $widgetData, key($k))
			            @endif
			        @endforeach
			    @empty
				    <x-filament::card>
				    	<h1>Please create a add some category</h1>
				    </x-filament::card>
			    @endforelse
		    </x-filament-support::grid>
	    @endif
